...
//listen method of express() gets two props one is port and other is callback function.And we execute this code then our server starts getiing requests and didn't stop it continuously go on accepting requests even if requests are coming or not.For closing this server on terminal we press ctrc+C.
app.listen(port, () => {
...
// express object contains a use method which is used to send response.It takes two params which are objects(In whatever form request came our express will parse it into an object so that our js code should understand this) one contains detail of request and with the help of other(response) wecan send response to that request.
app.use((req, res) => {
    console.log("Request received");
    //we can also print our request object
    console.log(req);//for seeeing this response on REST window we first need to run our code and also need to open it on browser by -> localhost:portNumber
    //our response object also contains many method int we can check them on express documentation.One of its method is send().With the help of this method we can send response back to client.
